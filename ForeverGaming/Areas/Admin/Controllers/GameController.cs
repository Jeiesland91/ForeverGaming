    ...
    public class GameController : Controller
    {
        ...
        public ViewResult Search()
        {
            ...
            if (search.HasSearchTerm)
            {
                ...
                {
                    Includes = "Genre, Type, Format"
                };
                if (search.IsGame)
                {
                    options.Where = g => g.Name.Contains(vm.SearchTerm);
                    vm.Header = $"Search results for game name '{vm.SearchTerm}'";
                }
                if (search.IsGenre)
                {
                    options.Where = g => g.Genre.Name.Contains(vm.SearchTerm);
                    vm.Header = $"Search results for genre name '{vm.SearchTerm}'";
                }
                if (search.IsType)
                {
                    options.Where = g => g.Type.Name.Contains(vm.SearchTerm);
                    vm.Header = $"Search results for type name '{vm.SearchTerm}'";
                }
                if (search.IsFormat)
                {
                    options.Where = g => g.Format.Name.Contains(vm.SearchTerm);
                    vm.Header = $"Search results for format name '{vm.SearchTerm}'";
                }
